...
const EditBook = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        const { title, author, publishYear, sellingPrice } = response.data;
        setTitle(title);
        setAuthor(author);
        setPublishYear(publishYear);
        setSellingPrice(sellingPrice); // Set selling price from fetched data
      })
      .catch((error) => {
        console.log(error);
  ...
  const handleUpdateBook = () => {
    const data = {
      ...
      author,
      publishYear,
      sellingPrice, // Include selling price in the data object
    };
    setLoading(true);
    axios
      .put(`http://localhost:5555/books/${id}`, data)
